...
{
    std::cout << getElement(aSeries, aLength); // wypisanie wskazanego elementu
}
...
void cTable::mPrintSeries(typeLoop aSeries)
{
    for (typeLoop i = 0; i < vLength; i++) // przejscie po wszystkich elementach w serii
    {
        ...
        std::cout << " "; // zrobienie odstepu miedzy wypisanymi elementami
    }
}
...
void cTable::mPrintTable()
{
    for (typeLoop i = 0; i < vSeries; i++) // przejscie po wszystkih seriach
    {
        ...
        std::cout << std::endl; // przejscie do kolejnej linii
    }
}
...
{
    std::ofstream StreamOut; // utworzenie strumienia plikowego
    StreamOut.open("table.txt"); // otwarcie pliku
    StreamOut << getSeries() << " " << getLength() << std::endl; // wypisanie
    for (typeLoop i = 0; i < vSeries; i++) // przejscie po wszystkich seriach
    {
        ...
        StreamOut << std::endl; // przejscie do nowej linii
    }
...
{
    std::ifstream StreamIn; // utworzenie strumienia plikowego
    ...
    StreamIn >> std::skipws >> aSize; // wczytanie liczby serii
    setSeries(aSize); // ustanowienie liczby serii
    StreamIn >> std::skipws >> aSize; // wczytywanie liczby elementow w serii
    ...
    for (typeLoop i = 0; i < vSeries; i++) // przejscie po wszystkich seriach
    {
        ...
        {
            StreamIn >> std::skipws >> aElement; // wczytywanie wartosci elementu
